...
El primer modelo es **Film**, que representa la entidad principal de la aplicación. Los atributos utilizados son *Título, Descripción, Género (Enum), Rating (Int), Duración (minutos) e Imagen (imageUri)*.  
Para la gestión de *imágenes* hemos optado por almacenar únicamente la URI en la base de datos lógica, evitando guardar binarios pesados dentro del archivo JSON y manteniendo las operaciones rápidas y ligeras.  
El segundo modelo es **Folder**, que permite la organización lógica de las películas. Sus atributos son *Nombre, Imagen* opcional e incluye una lista mutable de películas (films).   
La relación es de *“Uno-a-Muchos”* desde Folder hacia Film. Queríamos añadir que hemos aplicado una decisión de diseño similar a la versión iOS: si el usuario elimina una carpeta, las películas que hay dentro no se eliminan, sino que simplemente se desvinculan de la carpeta, preservando los datos del usuario.  
Por último, el tercer modelo es **Wish**, que gestiona una lista separada para las películas que el usuario pretende ver en el futuro. Sus atributos son *Título, Descripción, Género, Duración e Imagen opcional*. En lo que respecta a la estructura de la Interfaz de Usuario, decidimos que la aplicación adopta una navegación basada en Navigation-Compose y un NavHost integrado en la pantalla principal, gestionado mediante una barra inferior (Bottom Navigation Bar) que permite acceder a las distintas secciones.   
Las secciones de nuestra aplicación son:
- Library 
- All Films 
- Wishlist 
- Statistics 
- Add 
**Library** muestra las carpetas en una cuadrícula realizada con LazyVerticalGrid e implementa un modo de selección personalizado que permite la eliminación múltiple o individual de carpetas.  
Además utiliza el NavController para navegar al detalle de cada carpeta (accediendo específicamente a la vista FolderDetailScreen). 
**All Films** muestra una lista completa de todas las películas vistas, incluyendo la funcionalidad de filtros de ordenación por valoración, orden alfabético y fecha, con la adición de una barra de búsqueda para facilitar la gestión cuando se tienen muchas películas tras un uso prolongado de la aplicación. 
