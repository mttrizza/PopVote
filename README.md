...
La aplicación ha sido desarrollada utilizando las tecnologías más recientes del ecosistema Android para garantizar modernidad y mantenibilidad: el lenguaje es Kotlin, el framework de interfaz es Jetpack Compose, para la persistencia de datos hemos implementado un gestor propio basado en JSON mediante la clase StorageManager, y el entorno de desarrollo es Android Studio. La configuración apunta a compileSdk 34, con minSdk 24 y targetSdk 34. El corazón de la aplicación se basa en tres modelos de datos principales, definidos como data class en el archivo PopVoteModels.kt. Las relaciones han sido diseñadas para garantizar la integridad lógica de los datos. El primer modelo es Film, que representa la entidad principal de la aplicación. Los atributos utilizados son Título, Descripción, Género (Enum), Rating (Int), Duración (minutos) e Imagen (imageUri). Para la gestión de imágenes hemos optado por almacenar únicamente la URI en la base de datos lógica, evitando guardar binarios pesados dentro del archivo JSON y manteniendo las operaciones rápidas y ligeras. El segundo modelo es Folder, que permite la organización lógica de las películas. Sus atributos son Nombre, Imagen opcional e incluye una lista mutable de películas (films). La relación es de “Uno-a-Muchos” desde Folder hacia Film. Queríamos añadir que hemos aplicado una decisión de diseño similar a la versión iOS: si el usuario elimina una carpeta, las películas que hay dentro no se eliminan, sino que simplemente se desvinculan de la carpeta, preservando los datos del usuario. Por último, el tercer modelo es Wish, que gestiona una lista separada para las películas que el usuario pretende ver en el futuro. Sus atributos son Título, Descripción, Género, Duración e Imagen opcional. En lo que respecta a la estructura de la Interfaz de Usuario, decidimos que la aplicación adopta una navegación basada en Navigation-Compose y un NavHost integrado en la pantalla principal, gestionado mediante una barra inferior (Bottom Navigation Bar) que permite acceder a las distintas secciones. Las secciones de nuestra aplicación son: •	Library •	All Films •	Statistics •	Wishlist •	Add Library muestra las carpetas en una cuadrícula realizada con LazyVerticalGrid e implementa un modo de selección personalizado que permite la eliminación múltiple o individual de carpetas.
Además utiliza el NavController para navegar al detalle de cada carpeta (accediendo específicamente a la vista FolderDetailScreen). All Films muestra una lista completa de todas las películas vistas, incluyendo la funcionalidad de filtros de ordenación por valoración, orden alfabético y fecha, con la adición de una barra de búsqueda para facilitar la gestión cuando se tienen muchas películas tras un uso prolongado de la aplicación. Wishlist funciona de forma muy similar a la página All Films, pero en lugar de contener las películas ya vistas, permite añadir y marcar películas que se quieren ver en el futuro. Incluye un workflow “Mover a Biblioteca”, que implementa un botón (marca verde) que transfiere automáticamente un elemento de la Wishlist a la lista principal de películas, abriendo una ficha pre-rellenada. Statistics es nuestra penúltima sección, donde encontramos un panel analítico que calcula en tiempo real el tiempo total de visualización sumando la duración de todas las películas, así como nuestro género favorito, mediante un algoritmo que calcula la moda estadística de los géneros guardados. Finalmente, Add es el formulario completo para la introducción de datos, con validación de campos y selectores personalizados para duración y género. Queríamos informar también de algunos detalles de implementación relevantes, como el uso de mutableStateOf para la gestión reactiva del estado en la ViewModel y la sincronización automática con la interfaz. La arquitectura adoptada es MVVM simplificada: la ViewModel centraliza la lógica de negocio, expone el estado a las pantallas y se comunica directamente con el StorageManager para guardar y recuperar datos. Las imágenes se gestionan mediante la función copyImageToInternalStorage, que copia el archivo en la memoria interna y devuelve una URI local, utilizada por la interfaz para cargar la imagen con Coil. Por último, en cuanto a la UX y al Design System, hemos utilizado Material 3 para garantizar coherencia visual, componentes reutilizables y confirmaciones para acciones destructivas (AlertDialog), mejorando la experiencia del usuario y evitando errores accidentales, como borrar datos por equivocación. La solicitud de confirmación antes de eliminar mejora la experiencia del usuario y previene pérdidas de información. En cuanto al guardado de datos, hemos decidido utilizar persistencia local mediante un archivo JSON. La clase StorageManager se encarga de serializar y deserializar el objeto AppData con Gson, escribiéndolo en la memoria privada de la aplicación. Por lo tanto, no es necesaria una conexión a internet porque los datos se guardan en el disco del dispositivo. Lo hemos implementado por pasos: en primer lugar definimos los modelos de datos (Film, Folder y Wish), transformando clases normales de Kotlin en estructuras serializables. Después, en la ViewModel, centralizamos las operaciones de guardado y carga, invocando StorageManager cada vez que se produce un cambio relevante (añadir, editar, eliminar). Finalmente, para la gestión de imágenes, utilizamos URIs locales que apuntan a archivos internos, evitando sobrecargar el archivo JSON y manteniendo la aplicación ligera y eficiente.
